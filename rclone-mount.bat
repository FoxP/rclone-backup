...
SET RCLONE_VOLUME_NAME=pCloudEncrypted
...
SETLOCAL
	...
	%RCLONE_EXE_PATH% mount %RCLONE_REMOTE_PATH% %RCLONE_LOCAL_DRIVE% --volname=%RCLONE_VOLUME_NAME% --config=%RCLONE_CONFIG_PATH% %RCLONE_ADDITIONAL_FLAGS% --buffer-size %RCLONE_BUFFER_SIZE% --timeout %RCLONE_IO_IDLE_TIMEOUT% --contimeout %RCLONE_CONNECT_TIMEOUT% --low-level-retries %RCLONE_LOW_LEVEL_RETRIES_NUMBER% --retries %RCLONE_RETRIES_NUMBER% --retries-sleep %RCLONE_RETRIES_SLEEP%
ENDLOCAL
